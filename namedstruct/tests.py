...
class NamedstructTestCase(unittest.TestCase):
    def test_namedstruct(self):
        genDir = os.path.dirname(
            os.path.realpath(__file__)) + '/../localTestFiles/'
...
def generateTests():
    ...
    add(Struct("testStruct37")
        ...
        .addNullReference("bitFieldReference", BitField("bitField2000").add("aField", 0, 13).getType())
    ...
    GenderEnum = namedstruct.n_types.EnumType("GenderEnum", namedstruct.n_types.UINT8,
                                              {'MALE': 0, 'FEMALE': 1, 'OTHER': 2})
    NationEnum = namedstruct.n_types.EnumType("NationEnum", namedstruct.n_types.CHAR,
                                              {'ENGLISH': 'e', 'FRENCH': 'f', 'OTHER': 'a'})
    PartyEnum = namedstruct.n_types.IntEnumType("PartyEnum",
                                                {"CONSERVATIVE": -1, "LIBERAL": -2, "NDP": -3, "GREEN": -4})
    ...
    add(Struct("testPackOrder")
        .add("aardvark", Struct("Aardvark").addUInt8("weight", 5))
        .add("wombat", Struct("Wombat").addInt8("isABat", 0), pack_order=1)
        .add("zebra", Struct("Zebra").addUInt8("stripes", 96), pack_order=2)
        .add("walrus", (Struct("Walrus")
                        .add("subWalrus", namedstruct.Struct("SubWalrus")
                             .addUInt64("hugeNumber", 11111111)
                             ))
             , pack_order=1)
    ...
    try:
        add(Struct("testPackOrderMisuse")
            .add("pi", Int(3), pack_order=1)
            .addUInt8("g", Int(10)))
    ...
    header = namedstruct.generateHeader(testStructs, pool, includeSetters=True,
                                        namespace="namedStructTest", define="__NAMEDSTRUCTTEST__",
                                        headText="/* Testing structs generated by namedstructpy */")
    with open(os.path.dirname(os.path.realpath(__file__)) + "/../include/namedstruct/namedStructTests.h", "w") as f:
        f.write(header)
